...
public:
	...
	virtual bool			MountMapArchive( const char *relativePath );
	virtual void			UnmountMapArchive( void );
...
private:
	...
	searchpath_t *			mapArchiveSearchPath;	// transient maps/path/map.zip lightmap data
...
/*
===============
idFileSystemLocal::MountMapArchive

Mounts a loose per-map ZIP ahead of the normal search paths.  These archives
are deliberately transient and never participate in pure-server checks.
===============
*/
bool idFileSystemLocal::MountMapArchive( const char *relativePath ) {
	searchpath_t *search;
	pack_t *pak = NULL;
	idStr fullpath;
	...
	// dmap writes to fs_devpath, so try it first even when it is not represented
	// by the first directory node in the current search list.
	fullpath = BuildOSPath( fs_devpath.GetString(), gameFolder, relativePath );
	pak = LoadZipFile( fullpath );

	for ( search = searchPaths; !pak && search; search = search->next ) {
		if ( !search->dir ) {
			continue;
		}
		fullpath = BuildOSPath( search->dir->path, search->dir->gamedir, relativePath );
		pak = LoadZipFile( fullpath );
	}
	...
	pak->referenced = false;
	pak->isNew = false;

	mapArchiveSearchPath = new searchpath_t;
	mapArchiveSearchPath->dir = NULL;
	mapArchiveSearchPath->pack = pak;
	mapArchiveSearchPath->next = searchPaths;
	searchPaths = mapArchiveSearchPath;
	common->Printf( "Mounted map lightmap archive %s\n", pak->pakFilename.c_str() );
	return true;
}

/*
===============
idFileSystemLocal::UnmountMapArchive
===============
*/
void idFileSystemLocal::UnmountMapArchive( void ) {
	searchpath_t **link;
	searchpath_t *search;

	if ( !mapArchiveSearchPath ) {
		return;
	}

	for ( link = &searchPaths; *link && *link != mapArchiveSearchPath; link = &( *link )->next ) {
	}
	if ( *link == mapArchiveSearchPath ) {
		*link = mapArchiveSearchPath->next;
	}

	search = mapArchiveSearchPath;
	mapArchiveSearchPath = NULL;
	if ( search->pack ) {
		common->Printf( "Unmounted map lightmap archive %s\n", search->pack->pakFilename.c_str() );
		unzClose( search->pack->handle );
		delete [] search->pack->buildBuffer;
		if ( search->pack->addon_info ) {
			search->pack->addon_info->mapDecls.DeleteContents( true );
			delete search->pack->addon_info;
		}
		delete search->pack;
	}
	delete search;
...
void idFileSystemLocal::Shutdown( bool reloading ) {
	...
	ClearDirCache();
	// The transient archive is part of searchPaths, so the normal loop below
	// owns its storage from here on.
	mapArchiveSearchPath = NULL;
