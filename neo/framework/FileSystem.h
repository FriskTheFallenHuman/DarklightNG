...
public:
	...
	virtual int				AddZipFile( const char *path ) = 0;
							// Mount/unmount the loose per-map lightmap archive at the head of the search path.
	virtual bool			MountMapArchive( const char *relativePath ) = 0;
	virtual void			UnmountMapArchive( void ) = 0;
