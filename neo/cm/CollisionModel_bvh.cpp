...
This file is part of the Doom 3 GPL Source Code and is distributed under
...
void idCollisionModelManagerLocal::BuildModelBVHs( cm_model_t *model ) {
	...
	if ( refs.Num() > 0 ) {
		...
		model->polygonBvh.nodes = (cm_bvhNode_t *)Mem_ClearedAlloc( maxNodes * sizeof( cm_bvhNode_t ) );
		model->polygonBvh.polygons = (cm_polygon_t **)Mem_Alloc( refs.Num() * sizeof( cm_polygon_t * ) );
	...
	if ( refs.Num() > 0 ) {
		...
		model->brushBvh.nodes = (cm_bvhNode_t *)Mem_ClearedAlloc( maxNodes * sizeof( cm_bvhNode_t ) );
		model->brushBvh.brushes = (cm_brush_t **)Mem_Alloc( refs.Num() * sizeof( cm_brush_t * ) );
