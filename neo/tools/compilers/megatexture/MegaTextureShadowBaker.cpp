...
int staticShadowScene_t::FindOrCreateAlphaMask( const idMaterial *material ) {
	...
	for ( int stageIndex = 0; stageIndex < material->GetNumStages(); ++stageIndex ) {
		...
		const float alphaScale = idMath::ClampFloat( 0.0f, 1.0f, registers[stage->color.registers[3]] );
		...
		R_LoadImageProgram( stage->texture.image->imgName, &pixels, &width, &height, NULL );
		if ( !pixels || width <= 0 || height <= 0 ) {
			if ( pixels ) R_StaticFree( pixels );
			mask.opaqueFallback = true;
			break;
		}
		...
		for ( int surfaceIndex = 0; surfaceIndex < model->NumSurfaces(); ++surfaceIndex ) {
			...
			for ( int index = 0; index + 2 < geometry->numIndexes; index += 3 ) {
				...
				const idVec3 point[3] = {
					renderEntity.GetOrigin() + geometry->verts[vertex[0]].xyz * renderEntity.GetAxis(),
					renderEntity.GetOrigin() + geometry->verts[vertex[1]].xyz * renderEntity.GetAxis(),
					renderEntity.GetOrigin() + geometry->verts[vertex[2]].xyz * renderEntity.GetAxis()
				};
				...
				for ( int corner = 0; corner < 3; ++corner ) {
					...
					triangle.textureCoords[corner] = geometry->verts[vertex[corner]].st;
				}
