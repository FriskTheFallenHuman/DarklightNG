...
====================
*/
static void WriteUTriangles( const srfTriangles_t *uTris, int lightmapAtlas, const idList<idVec2> &lightmapTexCoords ) {
	int			col;
	int			i;

	// emit this chain
	procFile->WriteFloatString( "/* lightmapAtlas = */ %i /* numVerts = */ %i /* numIndexes = */ %i\n", 
		lightmapAtlas, uTris->numVerts, uTris->numIndexes );
	...
	for ( i = 0 ; i < uTris->numVerts ; i++ ) {
		...
		vec[6] = dv->normal[1];
		vec[7] = dv->normal[2];
		vec[8] = lightmapTexCoords[i][0];
		vec[9] = lightmapTexCoords[i][1];
		Write1DMatrix( procFile, 10, vec );
...
typedef struct interactionTris_s {
	...
	for ( group = area->groups ; group ; group = group->nextGroup ) {
		...
		const int lightmapAtlas = Lightmap_AddSurface( entityNum, group->material, uTri, lightmapTexCoords );
		WriteUTriangles( uTri, lightmapAtlas, lightmapTexCoords );
...
void WriteOutputFile( void ) {
	...
	Lightmap_Begin( dmapGlobals.mapFileBase );
