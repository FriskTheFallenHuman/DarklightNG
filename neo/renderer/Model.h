...
typedef struct srfTriangles_s {
	...
	idVec2 *					lightmapTexCoords;		// secondary UV stream loaded from mapProcFile004
	int						lightmapAtlas;			// -1 when this surface has no baked lighting
	idImage *					bakedLightmap;			// atlas irradiance texture
	idImage *					bakedDeluxemap;			// atlas tangent-space dominant light direction
...
public:
	...
	virtual idBounds			Bounds( const idRenderEntity *ent = NULL ) const = 0;
	...
	virtual idRenderModel *		InstantiateDynamicModel( const idRenderEntity *ent, const struct viewDef_s *view, idRenderModel *cachedModel ) = 0;
