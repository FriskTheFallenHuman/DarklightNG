...
int R_TriSurfMemory( const srfTriangles_t *tri ) {
	...
	if ( tri->lightmapTexCoords != NULL ) {
		total += tri->numVerts * sizeof( tri->lightmapTexCoords[0] );
	}
...
void R_ReallyFreeStaticTriSurf( srfTriangles_t *tri ) {
	...
	if ( tri->lightmapTexCoords != NULL ) {
		R_StaticFree( tri->lightmapTexCoords );
		tri->lightmapTexCoords = NULL;
	}
...
srfTriangles_t *R_CopyStaticTriSurf( const srfTriangles_t *tri ) {
	...
	newTri->lightmapAtlas = tri->lightmapAtlas;
	newTri->bakedLightmap = tri->bakedLightmap;
	newTri->bakedDeluxemap = tri->bakedDeluxemap;
	if ( tri->lightmapTexCoords ) {
		newTri->lightmapTexCoords = (idVec2 *)R_StaticAlloc( tri->numVerts * sizeof( idVec2 ) );
		memcpy( newTri->lightmapTexCoords, tri->lightmapTexCoords, tri->numVerts * sizeof( idVec2 ) );
	}
...
static void	R_DuplicateMirroredVertexes( srfTriangles_t *tri ) {
	...
	// The secondary UVs are deliberately kept out of idDrawVert.  Keep the
	// parallel stream in lockstep when tangent generation duplicates vertices.
	if ( tri->lightmapTexCoords ) {
		idVec2 *oldLightmapTexCoords = tri->lightmapTexCoords;
		tri->lightmapTexCoords = (idVec2 *)R_StaticAlloc( totalVerts * sizeof( idVec2 ) );
		memcpy( tri->lightmapTexCoords, oldLightmapTexCoords, tri->numVerts * sizeof( idVec2 ) );
		R_StaticFree( oldLightmapTexCoords );
	}
	...
	for ( i = 0 ; i < tri->numVerts ; i++ ) {
		...
		if ( j ) {
			...
			if ( tri->lightmapTexCoords ) {
				tri->lightmapTexCoords[j] = tri->lightmapTexCoords[i];
			}
