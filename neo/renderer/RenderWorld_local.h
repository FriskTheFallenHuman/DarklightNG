...
typedef struct portalArea_s {
	...
	const idAmbientCubeMap *ambientCubeMap;	// directional ambient lighting selected by map marker
} portalArea_t;
...
public:
	...
	ID_TIME_T					mapArchiveTimeStamp;	// maps/path/name.zip generated by dmap
	ID_TIME_T					ambientMapTimeStamp;	// maps/path/name.ambient area-lighting sidecar
	bool						hasBakedLightmaps;
	int						bakedSurfaceCount;
	int						bakedBatchCount;
	int						bakedLightSuppressionCount;
	int						bakedLightCandidateCount;
	int						bakedLightMovedCount;
	bool						bakedDrawReported;
	bool						ambientCubeDrawReported;
	...
	idRenderModel *			ParseModel( idLexer *src, bool hasLightmapUVs );
	idRenderModel *			ParseShadowModel( idLexer *src );
	void					SetupAreaRefs();
	void					ParseInterAreaPortals( idLexer *src );
	void					ParseNodes( idLexer *src );
	void					LoadAmbientCubeMaps( const char *mapBaseName );
	const idAmbientCubeMap *AmbientCubeMapForEntity( const idRenderEntityLocal *def ) const;
