...
int idMaterial::ParseTerm( idLexer &src ) {
	...
	if ( !token.Icmp( "fragmentPrograms" ) ) {
		return GetExpressionConstant( (float) glConfig.glslAvailable );
	}
...
void idMaterial::ParseStage( idLexer &src, const textureRepeat_t trpDefault ) {
	...
	while ( 1 ) {
		...
		if ( !token.Icmp( "program" ) ) {
			if ( src.ReadTokenOnLine( &token ) ) {
				newStage.vertexProgram = R_FindGLSLShader( GL_VERTEX_SHADER, token.c_str() );
				newStage.fragmentProgram = R_FindGLSLShader( GL_FRAGMENT_SHADER, token.c_str() );
			}
			continue;
		}
		...
		if ( !token.Icmp( "megaTexture" ) ) {
			if ( src.ReadTokenOnLine( &token ) ) {
				...
				newStage.vertexProgram = R_FindGLSLShader( GL_VERTEX_SHADER, "megaTexture" );
				newStage.fragmentProgram = R_FindGLSLShader( GL_FRAGMENT_SHADER, "megaTexture" );
				continue;
			}
		}
