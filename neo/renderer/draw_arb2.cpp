...
static void RB_ARB2_DrawBakedStage( const drawSurf_t *surf, idImage *bumpImage, const idVec4 bumpMatrix[2],
	...
	case SVC_MODULATE:
		...
		break;
	case SVC_INVERSE_MODULATE:
		qglProgramEnvParameter4fvARB( GL_VERTEX_PROGRAM_ARB, 4, negativeOne );
		qglProgramEnvParameter4fvARB( GL_VERTEX_PROGRAM_ARB, 5, one );
		break;
	default:
		qglProgramEnvParameter4fvARB( GL_VERTEX_PROGRAM_ARB, 4, zero );
		qglProgramEnvParameter4fvARB( GL_VERTEX_PROGRAM_ARB, 5, one );
		break;
	}
...
/*
==================
RB_ARB2_DrawBakedSurface
==================
*/
static void RB_ARB2_DrawBakedSurface( const drawSurf_t *surf ) {
	const srfTriangles_t *tri = surf->geo;
	...
	qglVertexAttribPointerARB( 11, 3, GL_FLOAT, false, sizeof( idDrawVert ), ambient->normal.ToFloatPtr() );
	qglVertexAttribPointerARB( 10, 3, GL_FLOAT, false, sizeof( idDrawVert ), ambient->tangents[1].ToFloatPtr() );
	qglVertexAttribPointerARB( 9, 3, GL_FLOAT, false, sizeof( idDrawVert ), ambient->tangents[0].ToFloatPtr() );
	qglVertexAttribPointerARB( 8, 2, GL_FLOAT, false, sizeof( idDrawVert ), ambient->st.ToFloatPtr() );
...
void RB_ARB2_DrawBakedLightmaps( drawSurf_t **drawSurfs, int numDrawSurfs ) {
	...
	qglEnableVertexAttribArrayARB( 10 );
	qglEnableVertexAttribArrayARB( 11 );
	qglEnableVertexAttribArrayARB( 12 );
	qglEnableClientState( GL_COLOR_ARRAY );
	...
	RB_RenderDrawSurfListWithFunction( drawSurfs, numDrawSurfs, RB_ARB2_DrawBakedSurface );

	qglDisableClientState( GL_COLOR_ARRAY );
	qglDisableVertexAttribArrayARB( 12 );
	qglDisableVertexAttribArrayARB( 11 );
	qglDisableVertexAttribArrayARB( 10 );
	qglDisableVertexAttribArrayARB( 9 );
	qglDisableVertexAttribArrayARB( 8 );
...
static progDef_t	progs[MAX_GLPROGS] = {
	...
	{ GL_VERTEX_PROGRAM_ARB, VPROG_BAKED_LIGHT, "bakedLight.vfp" },
	{ GL_FRAGMENT_PROGRAM_ARB, FPROG_BAKED_LIGHT, "bakedLight.vfp" },
