...
void idInteraction::CreateInteraction( const idRenderModel *model ) {
	...
	const bool useBakedSurfaceLighting = lightDef->world && lightDef->world->hasBakedLightmaps &&
		!r_skipBakedLightmaps.GetBool() && tr.backEndRenderer == BE_ARB2 &&
		lightDef->parms.bakedLight && !lightDef->lightHasMoved;
	...
	for ( int c = 0 ; c < model->NumSurfaces() ; c++ ) {
		...
		// Static light and shadow contributions are already present in this
		// surface's atlas.  Suppress only this proc surface, not the entire light,
		// so unbaked and moving models continue to receive realtime interactions.
