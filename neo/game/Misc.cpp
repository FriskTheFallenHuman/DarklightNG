...
static const idMapFile *staticInlineMapFile;
static idStr staticInlineMapName;
static idStr staticInlineScriptText;
static idStrList staticInlineTargetNames;

static bool StaticInlineIsIdentifierCharacter( char c ) {
	return ( c >= 'a' && c <= 'z' ) || ( c >= 'A' && c <= 'Z' ) ||
		( c >= '0' && c <= '9' ) || c == '_';
}

static void StaticInlineLoadReferences( void ) {
	idMapFile *mapFile = gameLocal.GetLevelMap();
	const char *mapName = gameLocal.GetMapName();
	if ( mapFile == staticInlineMapFile && !staticInlineMapName.Icmp( mapName ) ) {
		return;
	}

	staticInlineMapFile = mapFile;
	staticInlineMapName = mapName;
	staticInlineScriptText.Clear();
	staticInlineTargetNames.Clear();
	if ( !mapFile || mapFile->GetNumEntities() == 0 ) {
		return;
	}

	const idMapEntity *worldEntity = mapFile->GetEntity( 0 );
	idStr call = worldEntity->epairs.GetString( "call", "" );
	...
	if ( namespaceEnd > 0 ) {
		...
		if ( scriptLength > 0 && scriptBuffer ) {
			staticInlineScriptText = (const char *)scriptBuffer;
			fileSystem->FreeFile( scriptBuffer );
		}
	}

	for ( int entityNum = 0; entityNum < mapFile->GetNumEntities(); entityNum++ ) {
		const idDict &args = mapFile->GetEntity( entityNum )->epairs;
		for ( int keyNum = 0; keyNum < args.GetNumKeyVals(); keyNum++ ) {
			const idKeyValue *keyValue = args.GetKeyVal( keyNum );
			if ( !keyValue->GetKey().Icmpn( "target", 6 ) && keyValue->GetValue().Length() &&
				staticInlineTargetNames.FindIndex( keyValue->GetValue() ) < 0 ) {
				staticInlineTargetNames.Append( keyValue->GetValue() );
			}
		}
	}
}

static bool StaticInlineScriptMentionsEntity( const char *entityName ) {
	if ( !entityName || !entityName[0] ) {
		return false;
	}
	const char *text = staticInlineScriptText.c_str();
	...
	for ( const char *match = strstr( text, entityName ); match; match = strstr( match + nameLength, entityName ) ) {
		const bool validStart = match == text || !StaticInlineIsIdentifierCharacter( match[-1] );
		const bool validEnd = !StaticInlineIsIdentifierCharacter( match[nameLength] );
...
static bool StaticInlineHasIncomingTarget( const char *entityName ) {
	for ( int i = 0; i < staticInlineTargetNames.Num(); i++ ) {
		if ( !staticInlineTargetNames[i].Icmp( entityName ) ) {
			return true;
		}
	}
	return false;
...
static bool StaticInlineHasRuntimeControls( const idDict &args ) {
...
static bool StaticInlineShouldBake( const idDict &args, const idRenderModel *model ) {
	if ( idStr::Icmp( args.GetString( "classname", "" ), "func_static" ) || !model ||
		model->IsDefaultModel() || model->IsDynamicModel() != DM_STATIC ) {
		return false;
	}
	const char *modelName = args.GetString( "model", "" );
	const char *entityName = args.GetString( "name", "" );
	if ( !modelName[0] || !idStr::Icmp( modelName, entityName ) ) {
		return false;
	}
	...
	StaticInlineLoadReferences();
	return !StaticInlineHasRuntimeControls( args ) &&
...
void idStaticEntity::Spawn( void ) {
	...
	// Dmap automatically folds immutable external func_static models into the
