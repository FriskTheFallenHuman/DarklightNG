...
idClipBroadPhase::idClipBroadPhase( void ) {
...
void idClipModel::Save( idSaveGame *savefile ) const {
	...
	savefile->WriteInt( 0 );	// retained for save-game compatibility with the old sector touch counter
}
...
void idClipModel::Restore( idRestoreGame *savefile ) {
	...
	int oldTouchCount;
	savefile->ReadInt( oldTouchCount );
