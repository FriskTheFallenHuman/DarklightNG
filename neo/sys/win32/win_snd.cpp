...
static HRESULT Doom3DXTraceError( const char *operation, HRESULT result ) {
	char message[256];
	idStr::snPrintf( message, sizeof( message ), "%s failed with HRESULT 0x%08x\n", operation, result );
	OutputDebugStringA( message );
	return result;
}

#define DXTRACE_ERR(operation, result) Doom3DXTraceError(operation, result)
