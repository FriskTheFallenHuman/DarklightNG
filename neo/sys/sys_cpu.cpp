...
double Sys_GetClockTicks( void ) {
	...
	return (double)lowPart + 4294967296.0 * (double)highPart;
}
...
double Sys_ClockTicksPerSecond( void ) {
	...
	if ( RegOpenKeyExA( HKEY_LOCAL_MACHINE,
		"HARDWARE\\DESCRIPTION\\System\\CentralProcessor\\0", 0, KEY_READ, &processorKey ) == ERROR_SUCCESS ) {
		DWORD processorMHz = 0;
		DWORD valueType = 0;
		DWORD valueSize = sizeof( processorMHz );
		if ( RegQueryValueExA( processorKey, "~MHz", NULL, &valueType,
			(LPBYTE)&processorMHz, &valueSize ) == ERROR_SUCCESS && processorMHz != 0 ) {
			ticksPerSecond = (double)processorMHz * 1000000.0;
		}
	...
	if ( ticksPerSecond == 0.0 ) {
		...
		const Uint64 counterStart = SDL_GetPerformanceCounter();
		const double tickStart = Sys_GetClockTicks();
		SDL_Delay( 50 );
		const Uint64 counterElapsed = SDL_GetPerformanceCounter() - counterStart;
		const double ticksElapsed = Sys_GetClockTicks() - tickStart;
		if ( counterElapsed != 0 ) {
			ticksPerSecond = ticksElapsed * (double)counterFrequency / (double)counterElapsed;
		}
	}
