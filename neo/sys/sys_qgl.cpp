...
void QGL_Shutdown( void )
{
	...
	if ( qglLibraryLoaded ) {
		common->Printf( "...unloading OpenGL through SDL2\n" );
		SDL_GL_UnloadLibrary();
		qglLibraryLoaded = false;
	}
...
{
	assert( !qglLibraryLoaded );

	common->Printf( "...initializing QGL\n" );

	common->Printf( "...calling SDL_GL_LoadLibrary( '%s' ): ",
		dllname != NULL ? dllname : "system default" );

	if ( SDL_GL_LoadLibrary( dllname ) != 0 ) {
		common->Printf( "failed: %s\n", SDL_GetError() );
		return false;
	}
	qglLibraryLoaded = true;
